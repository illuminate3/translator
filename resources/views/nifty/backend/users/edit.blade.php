    ...
    <div class="col-lg-12">
        <div class="box info">
            <header>
                ...
                <h5>Edit User</h5>
                <div class="toolbar">
                    <a class="btn btn-default btn-sm btn-flat" href="{{URL::to('dashboard/users/create')}}"><span class="fa fa-lock"></span> User Password</a>
                </div>                
            </header>
        </div><!-- /.box -->
    </div>
        ...
        {{ Form::model($thisUser, ['url' => 'dashboard/users/update', 'class' => 'form-horizontal', 'role' => 'form'] ) }}  
            <div class="col-md-9">                 
                <div class="form-group {{ $errors->first('first_name') ? 'has-error' : '' }}">
                    ...
                    <div class="col-sm-10">
                        {{ Form::text('first_name', Input::old('first_name'), ['id' => 'first_name', 'class' => 'form-control input-lg']) }}
                ...
                <div class="form-group {{ $errors->first('last_name') ? 'has-error' : '' }}">
                    ...
                    <div class="col-sm-10">
                        {{ Form::text('last_name', Input::old('last_name'), ['id' => 'last_name', 'class' => 'form-control input-lg']) }}
                    </div>                    
                </div>
                <div class="form-group {{ $errors->first('email') ? 'has-error' : '' }}">
                    {{ Form::label('email', $errors->first('email'), ['class' => 'control-label col-sm-2']) }}
                    <div class="col-sm-10">
                        {{ Form::text('email', Input::old('email'), ['id' => 'email', 'class' => 'form-control input-lg']) }}
                    </div>                    
                </div>  
<!--                 <div class="form-group {{ $errors->first('password') ? 'has-error' : '' }}">
                    {{ Form::label('password', $errors->first('password'), ['class' => 'control-label col-sm-2']) }}
                    <div class="col-sm-10">
                        {{ Form::password('password', ['id' => 'password', 'class' => 'form-control input-lg']) }}
                    </div>                    
                </div>
                <div class="form-group {{ $errors->first('password') ? 'has-error' : '' }}">
                    ...
                    <div class="col-sm-10">
                        {{ Form::password('password_confirmation', ['id' => 'password_confirmation', 'class' => 'form-control input-lg']) }}
                    </div>                    
                </div>  -->
